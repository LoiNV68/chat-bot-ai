# Backend Project Structure & Services
...
**File:** `app/db/models`
Thiết kế ORM Map chính xác với Schema PostgreSQL đã chốt.

### 2.1. Documents Model (Quản lý Version & Quyền)
...
### 3.1. Ingestion Service (`app/services/ingestion_service.py`)

Logic xử lý file đầu vào.

```python
class IngestionService:
    def __init__(self, vector_store: VectorStoreService, db: AsyncSession):
...
### 3.2. Chat RAG Engine (`app/services/chat_engine.py`)

Logic xử lý tìm kiếm và trả lời.

```python
class ChatService:
    async def chat(self, user_query: str, history: List[str], user_info: User):
        # Bước 1: Rewrite Query (Dành riêng cho ngữ cảnh đối thoại)
...
| Method | Endpoint                 | Body/Params                 | Description                           |
| ------ | ------------------------ | --------------------------- | ------------------------------------- |
| POST   | `/upload`                | file, effective_date, scope | Upload tài liệu (Multipart/Form-data) |
| GET    | `/?limit=10&active=true` |                             | Lấy danh sách tài liệu                |
| GET    | `/{id}/history`          |                             | Lấy các version cũ của 1 file         |
| POST   | `/{id}/rollback`         |                             | Khôi phục version cũ                  |

### 4.4. Chat (`/api/v1/chat`)

| Method | Endpoint      | Body/Params         | Description                      |
| ------ | ------------- | ------------------- | -------------------------------- |
| POST   | `/completion` | {query, history}    | Chat Stream (Server-Sent Events) |
| POST   | `/feedback`   | {session_id, score} | Gửi like/dislike                 |

### 4.5. Feedback Loop (`/api/v1/feedback`) - Dành cho Admin

| Method | Endpoint     | Body/Params           | Description                          |
| ------ | ------------ | --------------------- | ------------------------------------ |
| GET    | `/bad-cases` |                       | Lấy danh sách câu trả lời bị Dislike |
| POST   | `/correct`   | {id, chosen_response} | Admin nhập câu trả lời đúng          |
...
Sử dụng `OAuth2PasswordBearer`. Mọi request vào các route bảo mật đều phải qua hàm `get_current_user` để decode JWT và lấy thông tin user.

### 5.2. Async Database

Sử dụng `asyncpg` driver cho PostgreSQL để đảm bảo non-blocking I/O. Khi có 100 sinh viên chat cùng lúc, server không bị treo chờ Database.

### 5.3. Vector Connection

Sử dụng `QdrantClient(url=..., prefer_grpc=True)` để tối ưu tốc độ tìm kiếm.
